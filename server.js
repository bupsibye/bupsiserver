...
// === НАСТРОЙКИ ===
const BOT_TOKEN = '8212274685:AAEN_jjb3hUnVN9CxdR9lSrG416yQXmk4Tk'; // ← ЗАМЕНИ на свой
const WEBHOOK_URL = 'https://bupsiserver.onrender.com'; // ← Render URL

const bot = new TelegramBot(BOT_TOKEN, { polling: false });

// === ХРАНИЛИЩЕ (в продакшене — база) ===
const gifts = new Map();
let giftIdCounter = 1;
...
app.get('/set-webhook', async (req, res) => {
  ...
  res.send(`✅ Вебхук установлен: <a href="${WEBHOOK_URL}/bot${BOT_TOKEN}">${WEBHOOK_URL}/bot${BOT_TOKEN}</a>`);
});
...
bot.on('callback_query', async (query) => {
  ...
  if (data.startsWith('decline_exchange_')) {
    ...
    const sessionId = [...exchangeSessions.keys()].find(k => k.startsWith('ex_'));
...
// === API: Добавить подарок (в будущем — при покупке) ===
...
// === API: Подтвердить обмен (оба подтвердили) ===
